...
export default function HanberryFeaturesPage() {
  return (
    <div 
      className="min-h-screen relative overflow-hidden font-poppins bg-white md:py-16"
    ...
    >
      ...
      <div className="relative z-10 py-16 px-4 sm:px-8 lg:px-16">
        ...
        <div className="flex items-center justify-center gap-4 mb-16">
          <div className="flex-1 h-px bg-[#E7C9B6] max-w-[400px]"></div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-medium text-center text-black whitespace-nowrap">
            The Hanberry Features
          </h2>
          <div className="md:flex-1 h-px bg-[#E7C9B6] max-w-[400px]"></div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 mx-auto">
          {features.map((feature, index) => (
            <div key={index} className="flex flex-col gap-4">
              <motion.div
                className={`${index%2 === 0 ? 'text-start' : 'text-end md:mt-24'} py-2.5  font-semibold text-lg sm:text-xl`}
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true, amount: 0.5 }}
                variants={fadeInUp}
              >
                <span className='bg-[#E7C9B6] px-8 py-3 rounded-md text-white'>
                  {feature.title}
                </span>
              </motion.div>
              <p className={`${index%2 === 0 ? 'text-start' : 'text-end'} text-base sm:text-lg text-gray-800 leading-relaxed`}>
                {feature.text}
              </p>
            </div>
          ))}
